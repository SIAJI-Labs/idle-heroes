@extends('layouts.app', [
    'wdashboard' => true,
    'wsecond_title' => 'Guild War',
    'wsidebar_menu' => 'guild-war',
    'wsidebar_submenu' => null,
    'wheader' => [
        'header_title' => 'Game Mode: Guild War',
        'header_breadcrumb' => [
            [
                'title' => 'Dashboard',
                'icon' => null,
...
@section('content')
    <div class="row">
        <div class="col-12 col-lg-4">
            <form class="card tw__sticky tw__top-40" id="form" method="POST" action="{{ route('s.game-mode.guild-war.store') }}">
                @csrf
                @method('POST')
        ...
        <div class="col-12 col-lg-8">
            <div class="card">
                ...
                <div class="card-body" id="guild_war-container"></div>
...
@section('js_inline')
    ...
    <script>
        ...
        const editData = (uuid) => {
            axios.get(`{{ route('s.game-mode.guild-war.index') }}/${uuid}`)
                .then(function (response) {
                    ...
                    // Reset Action URL
                    form.setAttribute('action', `{{ route('s.game-mode.guild-war.index') }}/${data.uuid}`);
                    // Set Method
        ...
        const resetAction = () => {
            ...
            if(document.getElementById(elId)){
                ...
                // Reset Action URL
                form.setAttribute('action', '{{ route('s.game-mode.guild-war.store') }}');
                // Set Method
                if(form.querySelector('input[name="_method"]')){
        ...
        const fetchData = (page = 1) => {
            if(document.getElementById('guild_war-container')){
                ...
                let url = new URL(`{{ route('s.json.game-mode.guild-war.list') }}`);
