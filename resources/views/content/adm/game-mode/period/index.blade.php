                        ...
                        if(data.length > 0){
                            data.forEach((val, index) => {
                                ...
                                content.innerHTML = `
                                    ...
                                    <small class="text-muted tw__italic tw__text-gray-300">Preparations end in <span data-start="${momentDateTime(moment(val.datetime), null, null, false)}" data-finish="${momentDateTime(moment(val.datetime).add(val.length, 'days'), null, null, false)}" data-length="${val.length}">-</span></small>
                                `;
        ...
        const editData = (uuid) => {
            axios.get(`{{ route('adm.game-mode.period.index') }}/${uuid}`)
                .then(function (response) {
                    ...
                    // Reset Period
                    if(flatpickrEl){
                        let formatedDatetime = data.datetime;
                        // Convert database datetime to current timezone
                        formatedDatetime = momentDateTime(moment(formatedDatetime), 'YYYY-MM-DD HH:mm', true);
                        flatpickrEl.setDate(formatedDatetime);
                    }
                    // Reset Length
